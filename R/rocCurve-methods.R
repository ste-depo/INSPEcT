...
setMethod('rocCurve', signature(object='INSPEcT_model', object2='INSPEcT_model'), 
	...
	{		
		...
		if( plot ) {
			...
			legendText <- c(legendText,paste(c('synthesis', 'degradation', 'processing'), ' - AIC', sep=''))
			...
			points(aCoordinatesAIC["specificity"],aCoordinatesAIC["sensitivity"],pch=17,col='red',cex=2.5)
			...
			points(bCoordinatesAIC["specificity"],bCoordinatesAIC["sensitivity"],pch=17,col='deepskyblue',cex=2.5)
			...
			points(cCoordinatesAIC["specificity"],cCoordinatesAIC["sensitivity"],pch=17,col='navy',cex=2.5)

			legend('bottomright', legend=legendText
				, col=rep(c('red', 'deepskyblue', 'navy'),3), lty=c(1,1,1,NA,NA,NA,NA,NA,NA), pch=c(NA,NA,NA,16,16,16,17,17,17), lwd=4)
		}
