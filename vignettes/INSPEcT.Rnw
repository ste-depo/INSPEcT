...
\Rpackage{INSPEcT} provides an R/Bioconductor compliant solution for the study of dynamic transcriptional regulatory processes. Based on RNA-seq data, \Rpackage{INSPEcT} determines mRNA synthesis, degradation and pre-mRNA processing rates over time for each gene, genome-wide. Finally, the \Rpackage{INSPEcT} modeling framework allows the identification of gene-level transcriptional regulatory mechanisms, determining which combination of sythesis, degradation and processing rates is most likely responsible for the observed mRNA level over time.

\Rpackage{INSPEcT} provides two diferent analysis approaches according to the available experimental data; total RNA-seq (no-4su configuration) eventually accompanied by 4su-seq (4su configuration). Regarding the second scenario, 4su-seq is a recent experimental technique developed to measure the concentration of nascent mRNA and for the genome-wide inference of gene-level synthesis rates. During a short pulse (typically few minutes), cells medium is complemented with 4su, a naturally occurring modified uridine that is incorporated within growing mRNA chains with minimal impact on cell viability. The chains which have incorporated the uridine variant (the newly synthesized ones) can be isolated from the total RNA population by biotinylation and purification with streptavidin-coated magnetic beads, followed by sequencing. In both cases, the main set of steps in the \Rpackage{INSPEcT} workflow is as follows:
\begin{itemize}
...
Whitin this vignette, some sections are explicitly divided in two parts, one for each aforementioned analysis configuration. However, the authors recomend at least on complete read of the document in order to understand the logic behind \Rpackage{INSPEcT} and the potential of the package. For details regarding \Rpackage{INSPEcT} extended methods description, refer to .... ... .
%
...
The \Rpackage{INSPEcT} framework includes function to quantify exon and intron features. \Rmethod{makeRPKMsFromBams} function builds an annotation for exons and introns, either at the level of transcripts or genes, and count reads falling on the two different annotations in each provided file in BAM or SAM format. This function prioritize the exon annotation, meaning that reads which fall on exon are not counted for introns, in case of overlap. Canonical RPKMs are then evaluated in order to provide expression values and they are returned as output together with read counts and annotations. \Rpackage{INSPEcT} also provides the oppurtunity to recompute RPKMs and counts through \Rpackage{DESeq2}, the experimental design is provided by the argument RCODEOPEN temporalDesign RCODECLOSE. If the \Rpackage{DESeq2} analysis is performed the package will be exploited also to compute the variances of the experimental data in the next steps of the pipeline; for this porpuse the output of the funcion contains the additional list of parameters RCODEOPEN dispersion\_parameters\_DESeq2 RCODECLOSE.

\subsubsection{4su working example}

<<features_quantification_4su_BAM, message=TRUE, eval=TRUE>>=
...
In case intronic and exonic counts have been computed differently they can be use to generate RPKMs through the function \Rmethod{makeRPKMsFromCounts} which differs from the aforementioned \Rmethod{makeRPKMsFromBams} just for the input. The user can also decide to provide for the following analyses RPKMs data.

<<features_quatification_4su_counts, message=TRUE, eval=TRUE>>=
...
makeRPKMsOut <- makeRPKMsFromCounts(txdb=txdb,allcounts=allcounts4su,temporalDesign=temporalDesign,DESeq2=TRUE)
...
<<features_quantification_No4su_BAM, message=TRUE, eval=TRUE>>=
...
<<features_quatification_No4su_counts, message=TRUE, eval=TRUE>>=
...
temporalDesign <- rep(c(0,1/6,1/3,1/2,1,1.5,2,4,8,12,16),3)
makeRPKMsOut_No4su <- makeRPKMsFromCounts(txdb=txdb,allcounts=allcountsNo4su,temporalDesign=temporalDesign,DESeq2=TRUE)
...
tL <- 1/6
mycerIds_4su <- newINSPEcT(tpts=tpts,labeling_time=tL
,rpkms_4su_exons=rpkms_4su$foursu_exons,rpkms_total_exons=rpkms_4su$total_exons
,rpkms_4su_introns=rpkms_4su$foursu_introns,rpkms_total_introns=rpkms_4su$total_introns
,dispersion_parameters_DESeq2=dispersion_parameters_DESeq2_4su
,varSamplingCondition=as.character(tpts[[1]]))

head(ratesFirstGuess(mycerIds_4su, 'synthesis'))
@

In case of a long 4su labeling time (longer than 10-15 minutes), it could be useful to activate the RCODEOPEN degDuringPulse RCODECLOSE option, as shown below. This option estimates all the rates of the RNA life-cycle without assuming that no degradation of the newly synthesized transcripts occurs during the pulse. The longer the labeling time is, the weaker this assumption gets. This option, however, involves solving a more complicated system of differential equations and for this reason it is not recommended for short labeling times. 
...
mycerIds10_4su <- mycerIds_4su[1:3]
...
tpts <- rep(c(0,1/6,1/3,1/2,1,1.5,2,4,8,12,16),3)
mycerIds_No4su <- newINSPEcT(tpts=tpts,labeling_time=NULL
,rpkms_4su_exons=NULL,rpkms_total_exons=rpkms_No4su$total_exons
,rpkms_4su_introns=NULL,rpkms_total_introns=rpkms_No4su$total_introns
,dispersion_parameters_DESeq2=dispersion_parameters_DESeq2_No4su)

head(ratesFirstGuess(mycerIds_No4su, 'synthesis'))
@
\begin{center}
...
mycerIds10_No4su <- mycerIds_No4su[1:3]
...
\begin{center}
<<post_model_heatmap, message=TRUE, eval=TRUE, fig.width=6, fig.height=4, fig.cap="Heatmap of the modeled rates representing the concentrations of total mRNA and pre-mRNA and the rates of the mRNA life cycle", fig.align="center", eval=TRUE>>=
# data('mycerIds10_4su', package='INSPEcT')
head(viewModelRates(mycerIds10_4su, 'synthesis'))
...
plotGene(mycerIds10_4su, 2, fix.yaxis=TRUE)
@
\end{center}
...
All the functionalities introduced in the previous subsection remain valid and the only difference in the outputs regards the absence of a variance associated with the syntehsis rate which is estimated directly from the 4su data.\\
The no-4su configuration provides the opportunity to optimize the parametric models under analysis through the direct numerical solution of the ODE system ($modelingParams(mycerIds10_No4su)\$estimateRatesWith="int"$) or exploiting the differentiability of the rates functions ($modelingParams(mycerIds10_No4su)\$estimateRatesWith="der"$). The latter drivative modelling is faster but less accurate, for further information see PUBBLICAZIONE.

<<model_rates_No4su, message=TRUE, eval=TRUE>>=
...
\begin{center}
<<post_model_heatmap_No4su, message=TRUE, eval=TRUE, fig.width=6, fig.height=4, fig.cap="Heatmap of the modeled rates representing the concentrations of total mRNA and pre-mRNA and the rates of the mRNA life cycle", fig.align="center", eval=TRUE>>=
# data('mycerIds10_No4su', package='INSPEcT')
head(viewModelRates(mycerIds10_No4su, 'synthesis'))
...
plotGene(mycerIds10_No4su, 2, fix.yaxis=TRUE)
@
\end{center}
...
# SOPPRIMERE I MESSAGGI DI LSODA
@
...
<<simulated_data_notrun2, message=TRUE, eval=FALSE>>=
...
# simData2rep_4su <- readRDS(system.file('extdata','simData2rep_4su.rds',package='INSPEcT'))
# simData3rep_4su <- readRDS(system.file('extdata','simData3rep_4su.rds',package='INSPEcT'))
...
As already mentioned in the introduction, the simulated data can be used also to evaluate the performances of the no-4su approach but, to be generated, they require the 4su data. For this reason, the only difference between the following examples and the code reported in the "4su working example" subsection is the argument RCODEOPEN No4su=TRUE RCODECLOSE of the method \Rmethod{makeSimDataset} which shapes the output of the function in a way suitable for an analysis without labeled transcripts.

<<simulated_data_notrun2_No4sU, message=TRUE, eval=FALSE>>=
...
@
\begin{center}
<<simulated_data_run_No4su, message=TRUE, eval=TRUE, fig.width=10, fig.height=5, fig.cap="For each rate, INSPEcT classification performance is measured in terms of sensitivity and specificity using a ROC curve analysis (rocCurve method). False negatives (FN) represent cases where the rate is identified as constant while it was simulated as varying. False positives (FP) represent cases where INSPEcT identified a rate as varying while it was simulated as constant. On the contrary, true positives (TP) and negatives (TN) are cases of correct classification of varying and constant rates, respectively. Consequently, sensitivity and specificity are computed using increasing thresholds for the Brown's method used to combine multiple p-values derived from the log-likelihood ratio tests", fig.align="center">>=
data('simRates', package='INSPEcT')
data('simData3rep_No4su', package='INSPEcT')
data('simData4rep_No4su', package='INSPEcT')

# simData2rep_No4su <- readRDS(system.file('extdata','simData2rep_No4su.rds',package='INSPEcT'))
# simData3rep_No4su <- readRDS(system.file('extdata','simData3rep_No4su.rds',package='INSPEcT'))
...
\begin{center}
<<simulated_data_run2_No4su, message=TRUE, eval=TRUE, fig.width=8, fig.height=4, fig.cap="Plot of the sensitivity (black curve) and specificity (red curve) that is achieved after performing the log-likelihood ratio and Brown's method for combining p-values with selected thresholds. Thresholds that can be set for chi-squared test to accept models that will undergo the log-likelihood ratio test and for Brown's p-value to assess variability of rates.">>=
